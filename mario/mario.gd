...
@onready var coin_counter := $VBoxContainer/CoinCounter
@onready var elevation_counter := $VBoxContainer/ElevationCounter
...
var finish_time_in_seconds := -1.0
var current_coin_count : int = 0

var start_time := 0.0
var elapsed_engine_ticks := 0
...
func _process(delta: float) -> void:
	...
	if position.y <= -32:
		if checkpoint_flag and is_instance_valid(checkpoint_flag):
			_fade_to_black_and_respawn_mario(true)
		elif !needs_respawning:
			needs_respawning = true
			_fade_to_black_and_respawn_mario()

	level_timer.visible = true
	var timer_seconds : float = float(elapsed_engine_ticks) / 30.0 + _time_since_last_tick
	if finish_time_in_seconds > 0:
		timer_seconds = finish_time_in_seconds
	...
	if Input.is_action_pressed("debug_restart"):
		if mario_inputs.stick or mario_inputs.button_a or mario_inputs.button_b or mario_inputs.button_z:
...
func _get_power_star(in_star_id : String) -> void:
	...
	saysound_playback.play_stream(preload("res://mario/star_get_socks.ogg"), 0, 0)
	#action = LibSM64.ACT_FALL_AFTER_STAR_GRAB
	set_angle(Quaternion.from_euler(camera.position - position).normalized())
	await get_tree().create_timer(1.5).timeout
	saysound_playback.play_stream(preload("res://mario/here_we_go.wav"), 0, 10)
...
func _respawn_mario() -> void:
	...
	SOGlobal.play_sound(preload("res://mario/enter_painting.WAV"), -8)
...
func _make_mario_inputs() -> LibSM64MarioInputs:
	...
	if _fading_out:
		var mario_inputs_cam_only := LibSM64MarioInputs.new()
		mario_inputs_cam_only.cam_look = mario_inputs.cam_look
		return mario_inputs_cam_only
...
func _fade_to_black_and_respawn_mario(to_checkpoint: bool = false) -> void:
	...
	if to_checkpoint:
		fader.fade_out(0.5, _restore_mario_to_checkpoint_and_fade_back_in)
	else:
		fader.fade_out(0.5, _respawn_mario_and_fade_back_in)

func _restore_mario_to_checkpoint_and_fade_back_in() -> void:
	_fading_out = false
	_restore_mario_to_checkpoint()
	fader.fade_in(0.5, null)

func _respawn_mario_and_fade_back_in() -> void:
	_fading_out = false
	_respawn_mario()
	fader.fade_in(0.5, null)
...
func _calculate_gameplay_camera(delta : float):
	...
	if Input.is_action_just_pressed("cam_stick_down"):
		if _cam_zoom < 2:
			_cam_zoom += 1
			SOGlobal.play_sound(preload("res://mario/sfx/sm64_camera_zoom_out.wav"), -3)
		else:
			SOGlobal.play_sound(preload("res://mario/sfx/sm64_camera_buzz.wav"), -6)
	if Input.is_action_just_pressed("cam_stick_up"):
		if _cam_zoom > 0:
			_cam_zoom -= 1
			SOGlobal.play_sound(preload("res://mario/sfx/sm64_camera_zoom_in.wav"), -3)
		else:
			SOGlobal.play_sound(preload("res://mario/sfx/sm64_camera_buzz.wav"), -6)
