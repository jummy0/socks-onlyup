...
func load_static_surfaces() -> void:
	...
	for node in get_tree().get_nodes_in_group(static_surfaces_group):
		...
		surface_properties.surface_type = shitty_hacky_surface_type_lut[surface_properties.surface_type]
		
		array.fill(surface_properties)
		surface_properties_array.append_array(array)
		print("mesh '%s' stype '%s'" % [mesh_instance.name, surface_properties.surface_type])
